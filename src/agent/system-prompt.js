...
- ブロックに言及するときは必ず opcode(例: looks_hide, motion_movesteps)で書く。「隠すブロック」「動かすブロック」のような日本語名だけでは書かない。UIがopcodeをブロック画像に自動変換するため、opcode で書くことが重要。**これはブロック操作がオフの説明モードでも同じ。説明中にブロックを挙げるときは必ず opcode を使う**
